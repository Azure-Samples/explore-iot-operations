...
. (Join-Path -Path $PSScriptRoot -ChildPath "Update-AioMqEndpointFile.ps1")
...
$aioMqHost = (Get-Content -Path (Join-Path -Path $PSScriptRoot -ChildPath ".config_aio_mq_host") -Raw).Trim()
Write-Host "AIO MQ host: $aioMqHost"

$aioMqPort = (Get-Content -Path (Join-Path -Path $PSScriptRoot -ChildPath ".config_aio_mq_port") -Raw).Trim()
