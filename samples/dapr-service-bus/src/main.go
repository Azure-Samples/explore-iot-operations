...
func eventHandler(c dapr.Client) common.TopicEventHandler {
	...
	return func(ctx context.Context, e *common.TopicEvent) (retry bool, err error) {
		...
		// Send to service bus
		BINDING_NAME := "servicebus-binding"
		BINDING_OPERATION := "create"
		in := &dapr.InvokeBindingRequest{Name: BINDING_NAME, Operation: BINDING_OPERATION, Data: []byte(e.RawData)}
		if err := c.InvokeOutputBinding(ctx, in); err != nil {
			panic(err)
		}
		log.Println("Send order to service bus")
